...
def valid_step(batch_valid, model):
    ...
    with torch.no_grad():
        logits = model(batch_input_ids, batch_attention_mask, batch_token_type_ids)
    X, Y, Z = metrics.get_evaluate_fpr(logits, batch_labels)
...
def valid(model, dataloader):
    ...
    if total_Y + total_Z > 0:
        f1 = 2 * total_X / (total_Y + total_Z)
    if total_Y > 0:
        precision = total_X / total_Y
    if total_Z > 0:
        recall = total_X / total_Z

    print("******************************************")
    print(f'precision: {precision}, recall: {recall}, f1: {f1}')
    print("******************************************")
    if config["logger"] == "wandb":
        logger.log({"valid_precision": precision, "valid_recall": recall, "valid_f1": f1})
    return f1
